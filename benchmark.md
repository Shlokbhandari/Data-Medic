...
|---|---|---|---|---|---|
| Duplicate transaction ID | Two rows share the same `transaction_id` | Duplicate `transaction_id` detected, likely caused by an upstream retry or resend | Remove the duplicate row, keep one | No duplicate `transaction_id`s remain, row count drops by exactly the number of duplicates | Detected (diagnosis and fix not built yet) |
