...
def test_missing_token_raises_error(sample_data):
    ...
    # Ensure environment is strictly empty for GITHUB_TOKEN and prevent .env from reloading
    with patch.dict(os.environ, {}, clear=True):
        with patch('github_pr.load_dotenv'):
...
def test_auth_failure_raises_permission_error(sample_data):
    patch_result, diagnosis, evidence, finding_type, validation_result = sample_data
    
    with patch.dict(os.environ, {'GITHUB_TOKEN': 'fake_token'}, clear=True):
        with patch('github_pr.load_dotenv'):
            with patch('github_pr.subprocess.run') as mock_run:
                mock_run.return_value.stdout = 'https://github.com/Shlokbhandari/Data-Medic.git'
                with patch('github_pr.Github') as mock_github:
                    mock_github.return_value.get_repo.side_effect = GithubException(401, "Unauthorized")
                    with pytest.raises(PermissionError, match="GITHUB_TOKEN is invalid or expired."):
                        push_and_create_pr('test-branch', patch_result, diagnosis, evidence, finding_type, validation_result)


def test_no_access_failure_raises_permission_error(sample_data):
    patch_result, diagnosis, evidence, finding_type, validation_result = sample_data
